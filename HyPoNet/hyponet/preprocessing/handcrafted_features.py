...
then saved as ``extracted_features.h5`` with dataset key ``features``. The
first four static columns are age, sex, height (cm), and weight (kg); BMI is
calculated from height and weight. Each fold's training split supplies the
imputation and scaling statistics for its validation and test splits.
"""
...
def compute_hemodynamic_features(abp_wave: np.ndarray, patient_data: np.ndarray, sampling_rate: int = 100) -> np.ndarray:
    ...
    if patient.shape != (5,):
        raise ValueError("Patient data must contain age, sex, height, weight, and BMI.")
    # A failed waveform calculation must not erase the five static variables.
    fallback = np.concatenate([np.zeros(9, dtype=np.float32), patient, np.zeros(8, dtype=np.float32)])
    try:
...
class MLFeatureExtractor:
    ...
    def extract_features(self, signals: np.ndarray | torch.Tensor, static: np.ndarray | torch.Tensor, standardize: bool = False) -> np.ndarray:
        """Extract raw features from signals with shape [N, 3, 6000] or [N, 6000, 3].

        Imputation and scaling require training-fold statistics and are applied
        by ``TrainingFeaturePreprocessor`` after extraction of each split.
        """

        if standardize:
            raise ValueError("Use TrainingFeaturePreprocessor fitted on the training split to standardize features.")
        ...
        if static.ndim != 2 or static.shape[0] != signals.shape[0] or static.shape[1] < 4:
            raise ValueError("static must have one row per signal with age, sex, height, and weight in its first four columns")
        # Derive BMI explicitly so the fifth descriptor is guaranteed to be
        # weight (kg) / height (m)^2, independent of the input array's fifth column.
        height_cm = static[:, 2].astype(np.float64)
        weight_kg = static[:, 3].astype(np.float64)
        with np.errstate(divide="ignore", invalid="ignore"):
            bmi = np.where((height_cm > 0) & (weight_kg > 0), weight_kg / (height_cm / 100.0) ** 2, np.nan)
        static = np.column_stack([static[:, :4], bmi]).astype(np.float32)
...
class TrainingFeaturePreprocessor:
    """Impute and scale all fold splits with training-only statistics."""

    def __init__(self) -> None:
        self.column_means: np.ndarray | None = None
        self.scaler: StandardScaler | None = None

    def fit(self, train_features: np.ndarray) -> "TrainingFeaturePreprocessor":
        train_features = np.asarray(train_features, dtype=np.float64)
        if train_features.ndim != 2 or train_features.shape[1] != FEATURE_DIM or len(train_features) == 0:
            raise ValueError(f"Training features must have nonempty shape [N, {FEATURE_DIM}], got {train_features.shape}")

        finite = np.isfinite(train_features)
        counts = finite.sum(axis=0)
        if np.any(counts == 0):
            columns = np.flatnonzero(counts == 0).tolist()
            raise ValueError(f"Training feature columns have no finite values: {columns}")
        self.column_means = np.where(finite, train_features, 0.0).sum(axis=0) / counts
        imputed = np.where(finite, train_features, self.column_means)
        self.scaler = StandardScaler().fit(imputed)
        return self

    def transform(self, features: np.ndarray) -> np.ndarray:
        if self.column_means is None or self.scaler is None:
            raise RuntimeError("Fit the feature preprocessor on training features before transforming a split.")
        features = np.asarray(features, dtype=np.float64)
        if features.ndim != 2 or features.shape[1] != FEATURE_DIM:
            raise ValueError(f"Features must have shape [N, {FEATURE_DIM}], got {features.shape}")
        if len(features) == 0:
            return np.empty((0, FEATURE_DIM), dtype=np.float32)
        imputed = np.where(np.isfinite(features), features, self.column_means)
        return ((imputed - self.scaler.mean_) / self.scaler.scale_).astype(np.float32)

    def save(self, path: Path) -> None:
        """Persist training-only statistics for chronological inference."""
        if self.column_means is None or self.scaler is None:
            raise RuntimeError("Fit the feature preprocessor before saving it.")
        np.savez(
            path,
            column_means=self.column_means,
            center=self.scaler.mean_,
            scale=self.scaler.scale_,
        )

    @classmethod
    def load(cls, path: Path) -> "TrainingFeaturePreprocessor":
        """Load training-only imputation and scaling parameters."""
        with np.load(path, allow_pickle=False) as saved:
            column_means = np.asarray(saved["column_means"], dtype=np.float64)
            center = np.asarray(saved["center"], dtype=np.float64)
            scale = np.asarray(saved["scale"], dtype=np.float64)
        if any(values.shape != (FEATURE_DIM,) for values in (column_means, center, scale)):
            raise ValueError(f"Feature preprocessor in {path} must contain {FEATURE_DIM} columns.")
        if not all(np.isfinite(values).all() for values in (column_means, center, scale)) or np.any(scale <= 0):
            raise ValueError(f"Feature preprocessor in {path} contains invalid statistics.")
        fitted = cls()
        fitted.column_means = column_means
        fitted.scaler = StandardScaler()
        fitted.scaler.mean_ = center
        fitted.scaler.scale_ = scale
        return fitted


def load_raw_features_for_split(split_dir: Path, static_data: np.ndarray) -> np.ndarray:
    """Read one split and extract its unfilled, unscaled 81-dimensional features."""
    ...
    with h5py.File(split_dir / "STATIC.h5", "r") as h5f:
        static_indices = h5f["STATIC"][:].astype(int).reshape(-1)

    selected_static = static_data[static_indices - 1]
    signals = np.stack([art, ecg, pleth], axis=1)
    return MLFeatureExtractor().extract_features(signals, selected_static)


def save_preprocessed_features(split_dir: Path, features: np.ndarray, output_name: str = "extracted_features.h5") -> Path:
    """Save preprocessed features under the HDF5 name and key used by training."""
...
def extract_features_for_split(
    ...
    output_name: str = "extracted_features.h5",
    *,
    preprocessor: TrainingFeaturePreprocessor | None = None,
) -> Path:
    """Save one split using statistics fitted on its sibling training split.

    A standalone validation/test call fits from ``../train``. Passing an
    already fitted preprocessor avoids repeating training feature extraction.
    """

    split_dir = Path(split_dir)
    if preprocessor is None:
        train_dir = split_dir.parent / "train"
        train_features = load_raw_features_for_split(train_dir, static_data)
        preprocessor = TrainingFeaturePreprocessor().fit(train_features)
        preprocessor.save(split_dir.parent / "feature_preprocessor.npz")
        raw_features = train_features if split_dir == train_dir else load_raw_features_for_split(split_dir, static_data)
    else:
        raw_features = load_raw_features_for_split(split_dir, static_data)
    return save_preprocessed_features(split_dir, preprocessor.transform(raw_features), output_name)
...
def extract_features_for_folds(data_root: Path, static_data_path: Path, folds: Iterable[int] = (1, 2, 3, 4, 5)) -> None:
    ...
    for fold in folds:
        ...
        if not train_dir.exists():
            if any((fold_dir / split).exists() for split in ("val", "test")):
                raise FileNotFoundError(f"Training split is required to preprocess fold {fold}: {train_dir}")
            continue

        train_features = load_raw_features_for_split(train_dir, static_data)
        preprocessor = TrainingFeaturePreprocessor().fit(train_features)
        preprocessor.save(fold_dir / "feature_preprocessor.npz")
        output_path = save_preprocessed_features(train_dir, preprocessor.transform(train_features))
        print(f"Saved {output_path}")
        for split in ("val", "test"):
            split_dir = fold_dir / split
            if split_dir.exists():
                output_path = extract_features_for_split(split_dir, static_data, preprocessor=preprocessor)
                print(f"Saved {output_path}")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--static-data", required=True, type=Path, help="NumPy file indexed by case ID - 1; first four columns: age, sex, height (cm), weight (kg). BMI is derived.")
